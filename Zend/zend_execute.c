...
static zend_always_inline void i_init_func_execute_data(zend_execute_data *execute_data, zend_op_array *op_array, zval *return_value) /* {{{ */
{
	...
	// if(EX(prev_execute_data)->func->type == ZEND_USER_FUNCTION){
	// 	// printf("PRE:%s," , (char *)(EX(prev_execute_data)->func->op_array.function_name->val));
	// }else{
	// 	printf("root,");
	// }
	// printf("%s\n" , (char *)(call->func->op_array.function_name->val));
	// printf("%s\n" , (char *)(op_array->function_name->val));
	/* Handle arguments */
